...
from tiny_build_profiles import (
    audit_flags, audit_int8_registrations, package_docs,
    ...
    for name in (
        subprocess.check_output(
            ["git", "ls-files", "--others", "--exclude-standard", "-z", "--",
             "source", "scripts", "tests", "CMakeLists.txt", "CMakePresets.json",
             "set_up_default_resources.py"], cwd=root
        )
        .decode()
        .split("\0")
    ):
