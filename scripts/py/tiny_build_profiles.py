...
def package_docs(root: Path, bundle: Path):
    """Include maintained instructions without historical measurement reports.
    ...
    text = (root / "docs/e8_tiny_build_run.md").read_text(encoding="utf-8")
    for name in ("e8_tiny_methodology.md", "e8_timer_validation.md"):
        shutil.copy2(root / "docs" / name, reference)
        text = text.replace(f"({name})", f"(reference/{name})")
